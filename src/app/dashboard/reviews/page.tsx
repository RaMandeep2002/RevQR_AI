    ...
    <div className="mx-auto max-w-7xl space-y-8">
      <div>
        ...
        <p className="mt-1 font-medium text-slate-500 dark:text-slate-300">
          See every review submitted by customers.
        </p>
      </div>
      <div className="grid gap-4">
        {reviews.map((review) => (
          <Card key={review.id} className="p-6 bg-white dark:dark:bg-[#0B1739] border-[#343B4F]/80">
            <div className="flex items-start justify-between gap-4">
              <div className="flex gap-4">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-slate-100 font-bold uppercase text-slate-500 dark:bg-slate-700 dark:text-slate-300">
                ...
                <div>
                  ...
                  <div className="mt-1 flex text-amber-400">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        className={`h-3 w-3 ${i < review.stars ? "fill-current" : "text-slate-200 dark:text-slate-700"}`}
                      />
                    ))}
                  </div>
                </div>
            ...
            <p className="mt-4 border-l-4 border-slate-100 pl-4 text-sm italic leading-relaxed text-slate-600 dark:border-slate-700 dark:text-slate-300">
              {review.review_text}
            </p>
          </Card>
        ))}
